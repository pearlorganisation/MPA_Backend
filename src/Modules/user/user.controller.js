...
// ==========================================
// CONFIGURATION: Kitni baar register karna hai yahan set karein
// ==========================================
const MAX_REGISTRATIONS_ALLOWED = 1; // Agar 2 likhoge toh 2 users register ho payenge
// ==========================================
...
export const registerUser = async (req, res) => {
  try {
    ...
    // 1. Registration limit check (Security Threat Trigger)
    const totalResearchers = await User.countDocuments({ role: "researcher" });

    if (totalResearchers >= MAX_REGISTRATIONS_ALLOWED) {
      // Professional Security Threat Response
      return res.status(403).json({
        success: false,
        isSecurityThreat: true,
        errorCode: "SEC_CRITICAL_ALERT_0x892",
        message: "CRITICAL ALERT: Unauthorized access blocked. Unusual registration volume detected from your IP. This activity has been flagged as a potential security breach and logged into our central firewall. Further attempts may result in permanent IP blacklisting.",
      });
    }
    ...
    // 3. Create user (PASSWORD WITHOUT BCRYPT AS REQUESTED)
    const user = await User.create({
      name,
      email,
      password: password, // Plain text password saved directly
      affiliation,
      role: "researcher",
    });
...
export const loginUser = async (req, res) => {
  try {
    ...
    // Matching plain text password (since bcrypt is removed)
...
export const createUser = async (req, res) => {
  try {
    ...
    // Plain text save for master admin creation as well
    const user = await User.create({
      name,
      email,
      password: password, 
...
export const getAllReviewers = async (req, res) => {
  try {
    const reviewer = await User.find({ role: "reviewer" }).select("-password");
    res.status(200).json({ success: true, count: reviewer.length, data: reviewer });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};